...
class TaskBlockParser:
    ...
    def parse(raw_block: RawBlock) -> TaskModel:
        ...
        if extraction.count > 0:
            file_path = (structure["file_path"] or "").strip().lower()
            # Rule:
            # - When multiple code blocks exist and target is .md,
            #   use content from the first ```markdown ... ``` block.
            # - Otherwise (non-.md or no markdown fence), use the largest block.
            if extraction.count > 1 and file_path.endswith(".md"):
                ...
                if md_content is not None:
                    ...
                    content = max(extraction.blocks, key=len)
            else:
                content = (
                    max(extraction.blocks, key=len)
                    if extraction.count > 1
                    else extraction.blocks[0]
                )
